...
def pay_with_paypal(request):
    ...
    # Pass the PayPal client ID to the template
    context = {
    ...
    response = render(request, "payment/pay_with_paypal.html", context)
...
def choose_payment_method(request):
    try:
        ...
        if order.status != 'Pending':
            return redirect("orders:order_history")  # Redirect if order is not in pending status

        if request.method == "POST":
            payment_method = request.POST.get('payment_method')
            if payment_method not in ['paypal', 'aba_bank', 'acleda_bank', 'wing_bank']:
                return render(request, 'confirm/choose_payment_method.html', {
            ...
            if payment_method == 'paypal':
                ...
                return redirect('orders:bank_account_details')
        else:
            return render(request, 'confirm/choose_payment_method.html')
...
def bank_account_details(request):
    try:
        ...
        if payment_method not in ['aba_bank', 'acleda_bank', 'wing_bank']:
            return redirect("orders:choose_payment_method")  # Redirect if payment method is not set

        # Calculate the total price
        total_price = order.total_price
        exchange_rate = 4100
        total_price_riel = Decimal(total_price) * Decimal(exchange_rate)
        
        # Format KHR amount
        if total_price_riel == total_price_riel.to_integral_value():
            total_price_riel_formatted = f"{total_price_riel:,.0f}"
        else:
            total_price_riel_formatted = f"{total_price_riel:,.2f}"
        ...
        return render(request, 'confirm/bank_account_details.html', {
            'payment_method': payment_method,
            'total_price': total_price,
            'total_price_riel': total_price_riel_formatted
        })
...
def confirm_address(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("orders:choose_payment_method")
    else:
...
class OrderHistoryImageView(View):
    ...
    def get(self, request, order_history_id):
        ...
        ordered_date_formatted = ordered_date_phnom_penh.strftime("%Y-%m-%d %I:%M %p")
